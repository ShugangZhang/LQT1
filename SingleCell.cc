...
// #include "SingleCell/CaseStudyRatLV.h"
// #include "SingleCell/TP06.h"
// #include "SingleCell/ToRORD.cc"
// #include "SingleCell/ORd.cc" 
#include "SingleCell/TPORd.cc" 
// #include "SingleCell/GattoniRat.h"
// #include "SingleCell/COGattoniRat.h"
// #include "SingleCell/CORatLV.h"
// #include "SingleCell/TerkRatLV.h"
...
int main(int argc, char *argv[])
{
	...
	double numS1 = 100; // 50 for short states. 49 for long states.
	double dt = 0.02; //ms
	double stopTime = numS1*BCL; //ms
	double stimStrength = -53;//-12;//-0.6e-3; // pA/pF   -6.0pA/pF(-0.6nA) for RatAtrial // -12.5pA/pF for CaMKII; -8.78pA for neonatalRatAtrial
	...
	// typedef ORd CellType;
	// typedef ORd* CellPointer;
	...
	CellPointer cell = new CellType(MCELL); // LVEPI for rats! EPI for human!

	// model-specific init
	FILE *datafile = fopen("Outputs/VentriSingleCellResults_LQT1_ISO_MCELL.dat","w+");
	...
	for(time = 0.0, step = 0; time < stopTime; time += dt, step++)
	{
		...
		if(step%(int(0.02/dt)) == 0) // 5*dt = 1ms once
		{
			// cell->outputAllStates(datafile);

			fprintf(datafile,"%4.10f\t", time);  // 1
			...
			// fprintf(datafile,"%4.10f\t", cell->getV()); // unit: mV
			// fprintf(datafile,"%4.10f\t", cell->getIKr()); // unit: 
			fprintf(datafile,"%4.10f\t", cell->getICaL()); // unit: 
			fprintf(datafile,"%4.10f\t", cell->getIKs()); // unit: 
			fprintf(datafile,"\n");
		...
		if (floor((time-stimStart)/BCL) == numS1 - 1)
		{
			if(cell->getV() > oldV)
			{
				...
				if( cell->getAbsDvdt() > dvdt_max )
				{
					...
					// cout << cell->getINa() << " " << cell->getIstim() << endl; 
				}
