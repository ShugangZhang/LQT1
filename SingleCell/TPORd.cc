...
public:
	...
	void setGKrFlag(int flag) {gKrFlag = flag;}
	...
	void update()
	{
		...
		*/
		...
		#ifdef BASELINE
		double dss = 1.0/(1.0+exp((-(v+3.940))/4.230)); // test Glory - original
		#endif
		#if defined CON || defined LQT1 
		// double dss = 1.0/(1.0+exp((-(v+3.940 - 20))/10.230)); // test Glory - case study
		double dss = 1.0/(1.0+exp((-(v+3.940))/4.230)); // test Glory - case study
		#endif
		#ifdef ISO1
		// dss = 1.0/(1.0+exp((-(v+3.940 - 8))/9.5)); // test Glory - case study - ISO
		dss = 1.0/(1.0+exp((-(v+3.940 + 12))/4.230)); // test Glory - case study - ISO
		#endif
		...
		#ifdef ISO1
		PCa *= 1.6; // 1.6 for 0D   // 3.5 for 1D to generate R-on-T
		#endif
		...
		// #ifdef LQT1   this is the fake heterogeneity by zhilin qu
		// if(gKrFlag == 0) GKr = 0.025;
		// if(gKrFlag == 1) GKr = 0.017;
		// #endif
		...
		double GKs = 0.0034;
		#else  // CON
		double GKs = 0.0034*sca; // O'hara et al. 2012. Heart Rhythm
		...
		#ifdef SUPREP
		double GKs_sup = GKs; // the current GKs to be supressed
		double GKs_rep = celltype == EPI? 1.4*0.0034:0.0034; // the replacing one
		double dose = 150; // fmol
		
		// PART I: the fraction of GKs_sup
		double frac_wt = 1.0; // the fraction of the original KCNQ1		
		if(dose >= 25)
