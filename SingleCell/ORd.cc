...
public:
	...
	void init(CellType ct)
	{
    	...
    	LC2  = 0;
    	LC1  = 0;
    	LO   = 1.0-UIC3-UIC2-UIF-UIM1-UIM2-UC3-UC2-UC1-UO-LC3-LC2-LC1;
	...
	void update()
	{
		...
		// ---------------------------- ORd Original INa ------------------------------
		#ifdef BASELINE
		double mss = 1.0/(1.0+exp((-(v+39.57))/9.871)); // ORd
		...
		// ------------------------- Markov INa ---------------------------
		#if defined CON || defined LQT3
	    //Compute INa
	    double a_11, a_12, a_13, a_2, a_3, a_4, a_5, b_11, b_12, b_13, b_2, b_3, b_4, b_5, u_1, u_2;
	    double UIC3_o, UIC2_o, UIF_o, UIM1_o, UIM2_o, UC3_o, UC2_o, UC1_o, UO_o, LC3_o, LC2_o, LC1_o, LO_o;
	    double UIC3_n, UIC2_n, UIF_n, UIM1_n, UIM2_n, UC3_n, UC2_n, UC1_n, UO_n, LC3_n, LC2_n, LC1_n, LO_n;
		...
		// clancy WT
		#ifdef CON
	    a_11 = 3.802/(0.1027*exp(-v/17.0) + 0.20*exp(-v/150));
	    a_12 = 3.802/(0.1027*exp(-v/15.0) + 0.23*exp(-v/150));
	    a_13 = 3.802/(0.1027*exp(-v/12.0) + 0.25*exp(-v/150));
	    b_11 = 0.77*exp(-v/20.3);
	    b_12 = 0.77*exp(-(v-5)/20.3);
	    b_13 = 0.17*exp(-(v-10)/20.3);
	    a_2 = 2.04*exp(v/29.68);
	    a_3 = 3.7933e-7*exp(-v/7.7); // +25 is key value for inactivation curve
	    b_3 = (0.0084 + 0.00002*v);
	    ...
	    b_11 = 0.77*exp(-v/20.3);
	    b_12 = 0.77*exp(-(v-5)/20.3);
	    b_13 = 0.535*exp(-(v-10)/20.3);
	    a_2 = 2.04*exp(v/29.68)/2.0; // Y1795C
	    a_3 = 3.7933e-7*exp(-v/7.7); // +25 is key value for inactivation curve
	    b_3 = (0.0084 + 0.00002*v);
	    b_2 = (a_13*a_2*a_3)/(b_13*b_3);
	    a_4 = a_2/100;
	    ...
	    #if defined CON || defined LQT3
	    const double co_UIC3 = 1./(1+dt*(a_11 + a_3));
	    const double co_UIC2 = 1./(1+dt*(b_11 + a_3 + a_12));
	    const double co_UIF = 1./(1+dt*(b_12 + a_3 + a_4 + b_2));
	    const double co_UIM1 = 1./(1+dt*(b_4 + a_5));
	    const double co_UIM2 = 1./(1+dt*b_5);
	    const double co_UC3 = 1./(1+dt*(b_3 + u_1 + a_11));
	    const double co_UC2 = 1./(1+dt*(b_3 + b_11 + u_1 + a_12));
	    const double co_UC1 = 1./(1+dt*(b_3 + b_12 + u_1 + a_13));
	    const double co_UO = 1./(1+dt*(a_2 + b_13 + u_1));
	    const double co_LC3 = 1./(1+dt*(u_2 + a_11));
	    const double co_LC2 = 1./(1+dt*(u_2 + b_11 + a_12));
	    const double co_LC1 = 1./(1+dt*(u_2 + b_12 + a_13));
	    const double co_LO = 1./(1+dt*(u_2 + b_13));
	    ...
	    {
	    	UIC3_n = co_UIC3 * (UIC3_o + dt*(UIC2*b_11 + UC3*b_3));
	    	UIC2_n = co_UIC2 * (UIC2_o + dt*(UIC3*a_11 + UIF*b_12 + UC2*b_3));
	    	UIF_n = co_UIF * (UIF_o + dt*(UIC2*a_12 + UIM1*b_4 + UC1*b_3 + UO*a_2));
	    	UIM1_n = co_UIM1 * (UIM1_o + dt*(UIF*a_4 + UIM2*b_5));
	    	UIM2_n = co_UIM2 * (UIM2_o + dt*(UIM1*a_5));

	    	UC3_n = co_UC3 * (UC3_o + dt*(UIC3*a_3 + LC3*u_2 + UC2*b_11));
	    	UC2_n = co_UC2 * (UC2_o + dt*(UIC2*a_3 + UC3*a_11 + UC1*b_12 + LC2*u_2));
	    	UC1_n = co_UC1 * (UC1_o + dt*(UIF*a_3 + UC2*a_12 + UO*b_13 + LC1*u_2));
	    	UO_n = co_UO * (UO_o + dt*(UIF*b_2 + UC1*a_13 + LO*u_2));
	    	
	    	LC3_n = co_LC3 * (LC3_o + dt*(UC3*u_1 + LC2*b_11));
	    	LC2_n = co_LC2 * (LC2_o + dt*(UC2*u_1 + LC3*a_11 + LC1*b_12));
	    	LC1_n = co_LC1 * (LC1_o + dt*(UC1*u_1 + LC2*a_12 + LO*b_13));
	    	LO_n = co_LO * (LO_o + dt*(UO*u_1 + LC1*a_13));

	    	err_sum =  fabs(UIC3 - UIC3_n)
	    			  +fabs(UIC2 - UIC2_n)
	    			  +fabs(UIF  - UIF_n )
	    			  +fabs(UIM1 - UIM1_n)
	    			  +fabs(UIM2 - UIM2_n)
	    	...
	    	// cout << iter <<  " Open probability: " << UO+LO << endl;
	    }
		...
		double tmL=1.0/(6.765*exp((v+11.64)/34.77)+8.552*exp(-(v+77.42)/5.955)); //original tmL = tm;
