...
const double D2 = 0.0385;
const double DD = 0.154 - 0.0385;

// const double D1 = 0.1171;
// const double D2 = 0.25*0.1171;
// const double DD = 0.75*0.1171;

// const double D1 = 0.1;
// const double D2 = 0.025;
// const double DD = 0.1 - 0.025;

// const double D1 = 0.154;
// const double D2 = 0.154;
// const double DD = 0.154 - 0.154;
...
const double D1 = 0.1;
const double D2 = 0.025;
const double DD = 0.1-0.025;//0.154 - 0.154;
**/
...
void writeData()
{
	...
	os << "Outputs2Dreal/ap_" << file_id << ".vtk";
...
{
	int _g = g[host_id];
	int _g_candidate = g[candidate_id];
	int temp;
	if (_g > 0)
	{
		// test code
...
__global__ void calc_du(int totalNum, double *d, double *u_v, double *xx, double *yy, double *zz, double *du, int *map, int *g, int *dev_rev_g)
{
	int id = blockIdx.x*blockDim.x + threadIdx.x;
	...
	if (id < totalNum)
	{
		...
		double dudxdy = (get_u_v(host_id + DimX + DimY, host_id, u_v, g) + get_u_v(host_id - DimX - DimY, host_id, u_v, g) - get_u_v(host_id - DimX + DimY, host_id, u_v, g) - get_u_v(host_id + DimX - DimY, host_id, u_v, g)) / (4 * dx*dx);
		...
		double dudy = (get_u_v(host_id + DimY, host_id, u_v, g) - get_u_v(host_id - DimY, host_id, u_v, g)) / (2 * dx);
...
__global__ void set_Istim(int totalNum, int b_totalNum, double timeoffset, double val, double time, double *u, float *b)
{
	...
	if (id < b_totalNum)
	{
		...
		if ((time >= (timeoffset + 0)) && (time<(timeoffset + 3)))
		{
			int targetIdx = (int)(b[id * 2]);
			u[Istim_idx + targetIdx*N] = val;
		}
	}
}
...
__global__ void Itot(int totalNum, double dt, double *u, short *typeArray, double *Itotr, double *u_v)
{
	...
	if (id < totalNum)
	{
		...
		// set a rectangular region of insult
		int gKrFlag = 0;		
		if(x_coor >= 150 && x_coor < 350 && y_coor >= 200 && y_coor < 300)
			gKrFlag = 1;
		else
			gKrFlag = 0;
...
__global__ void new_u_v(int num, double *u_v, double dt, double *du, double *itotr, short *type)
{
	int id = blockIdx.x*blockDim.x + threadIdx.x;
	...
	if (id < num)
	{
		...
		if (id == 1462422)
			printf("u_v = %f, du = %f, itot = %f", u_v[id],du[id],itotr[id]);
	}
}
...
int main(int argc, char **argv)
{
	...
	FILE *fp_ventricle;
	fp_ventricle = fopen("157layerwithfibervector.txt", "r");
	int num = 0;
	while (!feof(fp_ventricle))
	{	// the last line in the file MUST NOT be empty.
		int t1, t2, t5;
		float t3, t4;
		fscanf(fp_ventricle, "%d %d %f %f %d", &t1, &t2, &t3, &t4, &t5);
		...
		if(t1 == 0 || t1 == X-1 || t2 == 0 || t2 == Y-1)
			if (t5 != 0)  cout << "Tissue exists in geometry boundary!" << endl;
	...
	for (int x = 0; x < X; x++)
	{
		for (int y = 0; y < Y; y++)
		{
			if (g[x*DimX + y]>0)
			{
				g[x*DimX + y] = num1; // start from 1 NOT 0
				rev_g[num1] = x*DimX + y;
				num1++;
			}
		}
	...
	double *zz, *dev_zz;
	zz = new double[num];
	HANDLE_ERROR(cudaMalloc((void**)&dev_zz, sizeof(double)*num));
	...
	d = new double[num*10];
	double *dev_d;
	HANDLE_ERROR(cudaMalloc((void**)&dev_d, sizeof(double) * 10 * num));
	...
	cudaMalloc((void**)&dev_u, sizeof(double)*N*num);
	cudaMemcpy(dev_u, u, sizeof(double)*N*num, cudaMemcpyHostToDevice);
	...
	while (!feof(fp_ventricle))
	{
		int t1, t2, t5;
		double t3, t4;
		fscanf(fp_ventricle, "%d %d %lf %lf %d", &t1, &t2, &t3, &t4, &t5);

		int index = g[t1*DimX + t2*DimY] - 1;

		// set s2222222222222222 to be modified 
		// if((t1 >= 300 && t1 < 500) && (t2 >= 150 && t2 < 400) && (t5 <= 20 && t5 >= 18))
		// if((t1 >= 350 && t1 < 450) && (t2 >= 200 && t2 < 400) && (t5 <= 13 && t5 >= 11)) // it is ok for 280
		// if((t1 >= 450 && t1 < 550) && (t2 >= 300 && t2 < 400) && (t5 <= 20 && t5 >= 18)) 
		if((t1 >= 300 && t1 < 400) && (t2 >= 200 && t2 < 350) && (t5 <= 13 && t5 >= 11)) // its for all.
			is_s2[index] = 1;
		else
			is_s2[index] = 0;
		...
		// test island! Glory
		if(t5 >= 13 && t5 <= 16) // MID cells
			// if((t1 >= 600 && t1 < 689) && (t2 >= 200 && t2 < 487)) // try to shrink the area to test arrhy occur or not
				// t5 = t5;// remain the same
			if((t1 >= 550 && t1 < 689) && (t2 >= 150 && t2 < 487)) // the good one
				t5 = t5;// remain the same
			else if(t5 <= 14)
				t5 = 12;// set as ENDO
			else if(t5 > 14)
		...
		// calculate D
		d[index*5+1] = D2 + (DD*t3*t3);
		d[index*5+2] = DD*t3*t4;
		d[index*5+3] = DD*t3*t4;
		d[index*5+4] = D2 + (DD*t4*t4); 
	}
	cout << "Done" << endl;
	cout.flush();
	fclose(fp_ventricle);
	...
	// int cellid_test = g[409*DimX + 89*DimY + 2];
	// cout << "cellid:" << cellid_test << endl;
	// cout << "actual coors:" << 409*DimX + 89*DimY + 2 << endl;
	// cout << "coors from rev_g:" << rev_g[cellid_test] << endl;
	// cout << "xx " << xx[cellid_test-1] << endl; 
	// cout << "yy " << yy[cellid_test-1] << endl; 
	// cout << "zz " << zz[cellid_test-1] << endl; 
	...
	*/
	...
	{		
		...
		{			
				...
				if (g[x*DimX+y] == 0)
				{
					...
					if(x+1 >= 0) gg[5] = g[(x+1)*DimX + y];
					else gg[5] = 0;

					if(x-1 >= 0 && y+1 >= 0) gg[6] = g[(x-1)*DimX + (y+1)];
					else gg[6] = 0;
					if(y+1 >= 0) gg[7] = g[x*DimX + (y+1)];
					else gg[7] = 0;
					if(x+1 < X && y+1 >= 0) gg[8] = g[(x+1)*DimX + (y+1)];
					else gg[8] = 0;
				}
				...
				if (g[x*DimX + y] == 0 && isbound > 0)
				{
					...
					if (imax-0 < 0.0001) {cout << ir << ";" << il << endl; normalx = 0; normaly = 0; cout << "hit 000" << endl;} // imax is not int!!
					...
					g[x*DimX + y] = -g[(x+nnodex)*DimX+(y+nnodey)]; // 把最近的那个node的index取反存在自己里面
	...
	while (!feof(fp_stim))
	{
		...
		if (t3 == 157 && g[t1*DimX + t2*DimY] > 0)
		{
			new_b[num2 * 2] = (float)(g[t1*DimX + t2*DimY] - 1);
			new_b[num2 * 2 + 1] = t7;
			num2++;
		}
	}
	...
	while (time <= timedur)
	{
		...
		calc_du <<<(num + blockSize - 1) / blockSize, blockSize >>>(num, dev_d, dev_u_v, dev_xx, dev_yy, dev_zz, dev_du, dev_map, dev_g, dev_rev_g);
		...
		// if ((time >= BCL1+0) && (time <= BCL1+82)) //0-800
		// {
		// 	set_Istim <<<(num2 + blockSize - 1) / blockSize, blockSize >>>(num, num2, BCL1, -52, time, dev_u, dev_b);
		// 	cudaDeviceSynchronize();
		// }
		// s2222222222 to be modified

		// if(time >= 330 && time <= 332)  //   310 for CON   250 for SQT1   330 for QUIN 
		// {
		...
		new_u_v <<<(num + blockSize - 1) / blockSize, blockSize >>>(num, dev_u_v, dt, dev_du, dev_Itotr, dev_type); // Glory
		cudaDeviceSynchronize();
		...
		if(down_flag) 
		{
			// cout << "DW POINT NUMBER = " << oddnum << "; Vmin = " << vmin << "; time = " << time <<"; index = "<< small_index << endl;  // Glory
